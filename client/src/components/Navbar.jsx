  ...
  return (
    <div className={`flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 text-gray-600 
      border-b border-borderColor relative transition-all ${location.pathname === "/" && "bg-light"}`}>
       ...
       <Link to='/'>
       <img src={assets.logo} alt="logo" style={{ height: '60px', width: 'auto', transform: 'translate(-20px, -10px)'}} className="h-8 flex-shrink-0"/> 
       </Link>
       ...
       <div className= {`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor right-0 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 max-sm:p-4 transition-all duration-300 z-50 ${location.pathname === "/" ? "bg-light" : "bg-white"} ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"} `} >
        ...
        <div className='hidden lg:flex items-center text-sm gap-2 border border-borderColor px-3 rounded-full max-w-56' >
            <input type="text" className="py-1.5 w-full bg-trnsparent outline-none placeholder-grey-500" placeholder="Search Products"></input>
            <img src={assets.search_icon} alt="search" />
        </div>
        
        <div className="flex max-sm : flex-row items-start sm:items-center gap-6">
       ...
       <button className='sm:hidden cursor-pointer' aria-lable= "Menu" onClick={()=> setOpen(!open)}>
